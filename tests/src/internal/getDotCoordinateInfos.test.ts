...
describe(getDotCoordinateInfos.name, () => {
  it(`5 times 5 for one dot of radius 2`, () => {
    // Arrange
    const midCoordinate = new Coordinate(2, 2, 3);
    ...
    dot.radius = 2;
    // Act
    const result = getDotCoordinateInfos([dot], initialCoordinates);
    // Assert
  ...
  it(`5 times 5 for one dot of radius 1`, () => {
    for (let i = 0; i < 10; i++) {
        ...
        dot.radius = 1;
        // Act
        const infos = getDotCoordinateInfos([dot], initialCoordinates);
        // Assert
